...
class StreamServicePlivo extends EventEmitter {
  ...
  constructor(websocket, turnStateRef) {
    ...
    if (this.ws && typeof this.ws.on === 'function') {
      this.ws.on('error', (err) => {
        console.error('[StreamServicePlivo] WebSocket error:', err);
      });
    }
  ...
  sendAudioDirect(audio, audioDuration, hold = false, _srcTag, responseId) {
    ...
    console.log(`Sending audio directly for task: ${this.currentAudioTask}${isHoldMessage ? ' (Hold Message)' : ''}`);
    ...
    if (!this.assertTurnActive(scheduledTurn)) return;
    if (!this.ws || this.ws.readyState !== 1) return;
    if (this.ws.bufferedAmount && this.ws.bufferedAmount > 5000000) return;
    this.ws.send(
      JSON.stringify({
        streamId: this.streamId,
        event: 'checkpoint',
    ...
    // playedStream is now handled via the real Plivo callback in createCallSession.js
    // (was previously self-emitted here immediately, which defeated the purpose)
  ...
  playHoldMusicOnce() {
    ...
    const holdTaskId = `${scheduledTurn || 't'}_${uuid.v4()}`;
  ...
  stopCurrentAudio(cancelledResponseId) {
    ...
    // Always send clearAudio to Plivo regardless of currentAudioTask state.
    // During tail-playback (audio_done fired but caller still hearing audio),
    // currentAudioTask is null but Plivo may still be playing queued audio.
    this.stopPlayback();
    this.stopHoldMusic();
  }
  ...
  stopPlayback() {
    console.log('Forcing plivo to stop audio playback immediately.');
    if (!this.ws || this.ws.readyState !== 1 || !this.streamId) return;
...
module.exports = { StreamServicePlivo };
